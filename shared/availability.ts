...
function intervalsForAvailability(availability: Availability): Array<[number, number]> {
  return availability.windows.flatMap((window) => {
    ...
    const endDay =
      timeToMinutes(window.end) <= timeToMinutes(window.start) ? startDay + 1 : startDay;
    const startMs = localToUtcMs(
      2026,
      1,
      startDay,
      start.hour,
...
export function hasScheduleOverlap(
  ...
  return aIntervals.some(([aStart, aEnd]) =>
    bIntervals.some(
      ([bStart, bEnd]) => Math.min(aEnd, bEnd) - Math.max(aStart, bStart) >= minimumMinutes,
    ),
  );
}
